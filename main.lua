...
local SecurityLoader = loadstring(game:HttpGet("https://raw.githubusercontent.com/NYXOXOV3/gram/refs/heads/main/SecurityLoader.lua"))()
...
-- 🌙 NYX COLOR PALETTE (Goddess of Night)
local colors = {
    primary = Color3.fromRGB(168, 85, 247),      -- Moon violet
    secondary = Color3.fromRGB(99, 102, 241),    -- Night indigo
    accent = Color3.fromRGB(236, 72, 153),       -- Eclipse pink (rare use)

    success = Color3.fromRGB(34, 197, 94),
    warning = Color3.fromRGB(250, 204, 21),
    danger  = Color3.fromRGB(239, 68, 68),

    bg1 = Color3.fromRGB(5, 6, 18),     -- Deep night sky
    bg2 = Color3.fromRGB(9, 11, 30),    -- Dark void
    bg3 = Color3.fromRGB(15, 17, 45),   -- Soft night panel
    bg4 = Color3.fromRGB(25, 27, 60),   -- Highlight panel

    text = Color3.fromRGB(240, 240, 255),
    textDim = Color3.fromRGB(170, 170, 210),
    textDimmer = Color3.fromRGB(120, 120, 160),

    border = Color3.fromRGB(80, 85, 140),
...
local gradient = new("UIGradient",{
    ...
    Color=ColorSequence.new{
        ...
        ColorSequenceKeypoint.new(1, Color3.fromRGB(30, 27, 75))
    },
...
new("UIStroke",{
    ...
    Color=colors.primary,
    Thickness=1,
    Transparency=0.85  -- Lebih transparan dari 0.92
})
...
-- nyxGUI_v2.3.lua - Galaxy Edition (REFINED)
...
btnInfo.MouseButton1Click:Connect(function() switchPage("Info", "About nyx") end)
...
local function makeDropdown(parent, title, icon, items, onSelect, uniqueId)
    local dropdownFrame = new("Frame",{
        ...
        BackgroundColor3=colors.bg4,
        BackgroundTransparency=0.15,  -- Lebih transparan dari 0.4
        BorderSizePixel=0,
        AutomaticSize=Enum.AutomaticSize.Y,
        ZIndex=7,
...
local function makeButton(parent, label, callback)
    local btnFrame = new("Frame",{
        ...
        Size=UDim2.new(1, 0, 0, 32),
        BackgroundColor3=colors.primary,
        BackgroundTransparency = 0.15,  -- Lebih transparan dari 0.2
        BorderSizePixel=0,
        ZIndex=8
    })
...
FreecamModule.SetMainGuiName("nyxGUI_Galaxy")
...
makeToggle(catHideStats, "⚡ Enable Hide Stats", function(on)
    ...
    if on then
        HideStats.Enable()
        Notify.Send("Hide Stats ✨", "Hide Stats aktif! '-nyx-' berkilau di atas nama.", 3)
    else
        HideStats.Disable()
    ...
    if requestFunc then
        ...
        local testPayload = {
            embeds = {{
                ...
                description = "Your Discord webhook is working correctly!\n\nnyxx GUI is ready to send fish notifications." .. filterText,
                color = 3447003,
                footer = {
                    text = "nyxx Webhook Test • " .. os.date("%m/%d/%Y %H:%M"),
